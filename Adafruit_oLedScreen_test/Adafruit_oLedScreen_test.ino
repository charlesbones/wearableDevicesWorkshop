...
void lcdTestPattern(void)
{
  ...
  for (i = 0; i < 128; i++)
  {
    ...
    {
      if (i < 16) {
        tft.writeData(uint8_t(RED >> 8)); 
        tft.writeData(uint8_t(RED));
      }
      else if (i < 32) {
        tft.writeData(uint8_t(YELLOW >> 8)); 
        tft.writeData(uint8_t(YELLOW));
      }
      else if (i < 48) {
        tft.writeData(uint8_t(GREEN >> 8)); 
        tft.writeData(uint8_t(GREEN));
      }
      else if (i < 64) {
        tft.writeData(uint8_t(CYAN >> 8)); 
        tft.writeData(uint8_t(CYAN));
      }
      else if (i < 80) {
        tft.writeData(uint8_t(BLUE >> 8)); 
        tft.writeData(uint8_t(BLUE));
      }
      else if (i < 96) {
        tft.writeData(uint8_t(MAGENTA >> 8)); 
        tft.writeData(uint8_t(MAGENTA));
      }
      else if (i < 112) {
        tft.writeData(uint8_t(BLACK >> 8)); 
        tft.writeData(uint8_t(BLACK));
      }
      else {
        tft.writeData(uint8_t(WHITE >> 8)); 
        tft.writeData(uint8_t(WHITE));
      }
    }
  }
}
